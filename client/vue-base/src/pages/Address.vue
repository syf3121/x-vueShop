<template>
    ...
    <modal :mdShow="isAddMdShow" @close="closeAddModal">
      <ul slot='message'>
          <li>
              <label>收件人：<input type="text" v-model="address.userName"></label>
          </li>
          <li>
              <label>地址：<input type="text" v-model="address.streetName"></label>
          </li>
          <li>
              <label>电话：<input type="text" v-model="address.tel"></label>
          </li>
          <li>
              <label>邮编：<input type="text" v-model="address.postCode"></label>
          </li>
      </ul>
      <div slot="btnGroup">
          <a class="btn btn--m" href="javascript:;" @click="subAddress" >确认</a>
...
export default {
  ...
  methods:{
        ...
        this.$http.get('/users/addressList').then(res =>{
            ...
            if(res.status === '0'){
                ...
                this.addressList.forEach((item,index) =>{
                    if(item.isDefault){
                        let temp = this.addressList[0]
                        this.addressList[0] = this.addressList[index]
                        this.addressList[index] = temp
                    }
                })
            }
        })
    ...
    delAddress(){
        this.$http.post('/users/delAddress',{addressId:this.addressId}).then(res =>{
           res = res.data
           if(res.status === '0'){
               this.closeModal()
               this.init()
    ...
    subAddress(){
        this.$http.post('/users/addAddress',this.address).then(res =>{
            res = res.data
            if(res.status === '0'){
                this.closeAddModal()
                this.init()

...
export default {
        ...
        totalPrice(){
            ...
            this.cartList.forEach(item =>{
                if(parseInt(item.checked) === 1){
                   price += parseInt(item.salePrice) * parseInt(item.productNum)  
                } 
            })
            return price
        }
    ...
    methods:{
        ...
        delCart(){
            this.$http.post('/users/cartDel',{productId:this.delItem.productId}).then(res =>{
                res = res.data
                if(res.status === '0'){
                    this.modalConfirm = false
        ...
        editCart(flag,item){
            ...
            this.$http.post('/users/cartEdit',{productNum,productId,checked}).then(res =>{
                res = res.data
                if(res.status === '0'){
        ...
        toggleCheckAll(){
            ...
            this.$http.post('/users/editCheckAll',{checkAll:flag}).then(res =>{
                res = res.data
                if(res.status === '0'){

        ...
        <div class="price-count-wrap">
          <div class="price-count">
            <ul>
              ...
              <li>
                <span></span>
                <span class="red">满100包邮, 全场9折</span>
              </li>
            </ul>
          </div>
        </div>
  ...
  methods:{
      init(){
          this.$http.get('/users/cartList').then(res =>{
              ...
              this.orderTotal = this.shipping + this.subTotal - this.discount
          })
      },
      payMent(){
          let addressId = this.$route.query.addressId
          this.$http.post('/users/payMent',{addressId,orderTotal:this.orderTotal}).then(res =>{
              res = res.data
              if(res. status === '0'){
                  this.$router.push({
